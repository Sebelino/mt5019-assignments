...
to estimate the relationship between the dose and
...
\\begin{align*}
\\text{Where:} & \\\\
P(Y = 1 | X) & \\text{ is the probability of the dependent variable } Y \\text{ being 1, given the predictor } X. \\\\
\\beta_0 & \\text{ is the intercept term.} \\\\
\\beta_1 & \\text{ is the coefficient of the predictor variable.} \\\\
X & \\text{ is the predictor variable, log(dose).}
\\end{align*}
")
```
...
```
This means that, at a \(log(dose)\) of zero, the odds of developing a tumor are approximately 1.99.
...
The odds value of \(1.682\) means that for each one-unit increase in log(dose), the odds of developing a tumor increase by approximately 68.2%. 
...
In terms of log-odds, the intercept represents the log-odds of developing a tumor when \(\log(\text{dose}) = 0\).
...
The slope represents the change in log-odds of developing a tumor for each one-unit increase in \(\log(\text{dose})\).

### Confidence interval for the tumor risk at dose 0.25:
```{r}
probability_ci
```
This interval tells us that we are 95% confident that the true probability of developing a tumor at \(\log(\text{dose}) = -1.39\) lies between **0.394** (39.4%) and **0.588** (58.8%). 
